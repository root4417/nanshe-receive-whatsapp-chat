...
abstract class ReceiveWhatsappChat<T extends StatefulWidget> extends State<T> {
  ...
  Future<void> receiveShareIOS(String path) async {
    ...
    var dirName = path.split('/').last.split('.').first;
    var desPath = await createDestinationDir(dirName);
  ...
  Future<String> createDestinationDir(String dirName)async{
    var destination = (await getTemporaryDirectory()).path + '/' + dirName;
    var desDirectory = Directory(destination);
    if(await desDirectory.exists()){
      await desDirectory.delete(recursive: true);
    }
    await desDirectory.create();
    return desDirectory.path;
  }
